...
const handleStripeWebhookService = async (event: Stripe.Event) => {
  switch (event.type) {
    ...
    default:
      throw new AppError(StatusCodes.BAD_REQUEST, 'Invalid event type');
  }
};
...
const getSubscribtionService = async (userId: string) => {
  ...
  if (!subscription) {
    throw new AppError(StatusCodes.BAD_REQUEST, 'Subscribtion not found');
  }
  return subscription;
};
...
const cancelSubscriptation = async (userId: string) => {
  ...
  if (!updatedSubscription) {
    throw new AppError(StatusCodes.BAD_REQUEST, 'Subscribtion not cancel');
  }
...
const updateSubscriptionPlanService = async (
  ...
  if (!newPlan) {
    throw new AppError(StatusCodes.BAD_REQUEST, 'New plan not found');
  }
